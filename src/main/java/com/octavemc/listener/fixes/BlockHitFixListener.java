...
public class BlockHitFixListener implements Listener {
    ...
    private final TObjectLongMap<UUID> lastInteractTimes;
    ...
    public BlockHitFixListener() {
        ...
        this.lastInteractTimes = new TObjectLongHashMap<>();
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (event.isCancelled()
                && NON_TRANSPARENT_ATTACK_BREAK_TYPES.contains(event.getBlock().getType())) {
            cancelAttackingMillis(event.getPlayer().getUniqueId(), THRESHOLD);
        }
    }
    ...
    public void onEntityDamageByEntity(EntityDamageEvent event) {
        ...
        if (attacker != null
                && this.lastInteractTimes.get(attacker.getUniqueId()) != this.lastInteractTimes.getNoEntryValue()
                && this.lastInteractTimes.get(attacker.getUniqueId()) - System.currentTimeMillis() > 0L) {
