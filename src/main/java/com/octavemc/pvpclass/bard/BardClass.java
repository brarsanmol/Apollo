package com.octavemc.pvpclass.bard;
...
public class BardClass extends PvpClass implements Listener {
    ...
    public BardClass(Apollo plugin) {
        ...
        bardData.heldTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (held != null) {
                    ...
                    if (!Apollo.getInstance().getFactionDao().getFactionAt(player.getLocation()).get().isSafezone()) {
                        // Apply the held effect to faction members.
                        plugin.getFactionDao().getByPlayer(player.getUniqueId()).ifPresent(faction -> {
                            ...
                            for (Entity nearby : nearbyEntities) {
                                if (nearby instanceof Player target && !player.equals(target)) {
                ...
                if (energy != 0 && energy != lastEnergy && (energy % 10 == 0 || lastEnergy - energy - 1 > 0 || energy == BardData.MAX_ENERGY)) {
                    lastEnergy = energy;
                    player.sendMessage(Configuration.PRIMARY_MESSAGE_PREFIX + "Your bard energy is now " + ChatColor.AQUA + energy + ChatColor.GRAY + '.');
                }
            }
        ...
        if (newStack != null) {
            ...
            if (bardEffect != null) {
                ...
                player.sendMessage(ChatColor.YELLOW + "  Clickable Effect: " + ChatColor.AQUA +
                        //TODO: Do appropriate potion effect.
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
            ...
            if (bardData != null) {
                ...
                if (!plugin.getFactionDao().getFactionAt(player.getLocation()).get().isSafezone()) {
                    // Apply the held effect to faction members.
                    plugin.getFactionDao().getByPlayer(player.getUniqueId()).ifPresent(faction -> {
                        Collection<Entity> nearbyEntities = player.getNearbyEntities(TEAMMATE_NEARBY_RADIUS, TEAMMATE_NEARBY_RADIUS, TEAMMATE_NEARBY_RADIUS);
                        for (Entity nearby : nearbyEntities) {
    ...
    private boolean canUseBardEffect(Player player, BardData bardData, BardEffect bardEffect, boolean sendFeedback) {
        ...
        Faction factionAt = plugin.getFactionDao().getFactionAt(player.getLocation()).get();
