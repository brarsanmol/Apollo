...
import com.octavemc.visualise.VisualBlock;
...
public class SubclaimWandListener implements Listener {
    ...
    public SubclaimWandListener(Apollo plugin) {
        this.plugin = plugin;
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.LEFT_CLICK_BLOCK || action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (plugin.getClaimHandler().canSubclaimHere(player, blockLocation)) {
                ...
                if (selectionId == 1) claimSelection.setPositionOne(blockLocation);
                if (selectionId == 2) claimSelection.setPositionTwo(blockLocation);
